...
**Meta nodes** manage and monitor both the uptime of nodes in the cluster and distribution of [shards](/influxdb/v1.7/concepts/glossary/#shard) among nodes in the cluster. Meta nodes hold information about which data nodes own which shards; information that the [anti-entropy](/enterprise_influxdb/v1.7/administration/anti-entropy/) (AE) process depends on.
...
If all meta nodes in the cluster are fully functional, complete the following steps to [replace a meta node](#replace-meta-nodes-in-an-influxdb-enterprise-cluster).

### Replace an unresponsive meta node

If replacing a meta node that is either unreachable or unrecoverable, you must forcefully remove the node from the meta cluster. See [step 2.2](#2-2-remove-the-non-leader-meta-node) of the [replace meta nodes](#replace-meta-nodes-in-an-influxdb-enterprise-cluster) process.
