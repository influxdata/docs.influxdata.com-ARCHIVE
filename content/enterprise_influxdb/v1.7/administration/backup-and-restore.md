...
Before you restore, stop anti-entropy (AE) services if they are enabled (disabled by default). To successfully do this, complete the following steps.
1. For each data node in the cluster, do the following:
   a. Stop the influxdb service.
   b. Disable anti-entropy in influx.config.
   c. Restart the influxdb service, and wait for the data node to return to a healthy state (node is up, receiving read and write requests, and the hinted handoff queue is drained). 
