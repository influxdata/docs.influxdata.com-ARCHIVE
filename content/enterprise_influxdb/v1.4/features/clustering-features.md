...
aliases:
    - /enterprise/v1.4/features/clustering-features/
menu:
...
see [Query Management](/influxdb/v1.4/troubleshooting/query_management/).
...
[run-interval](/enterprise_influxdb/v1.4/administration/configuration#lease-duration--1m0s)
...
[lease-duration](/enterprise_influxdb/v1.4/administration/configuration#run-interval--1s)
...
* [Copy Shard](/enterprise_influxdb/v1.4/features/cluster-commands/#copy-shard) support - copy a shard from one node to another
* [Copy Shard Status](/enterprise_influxdb/v1.4/features/cluster-commands/#copy-shard-status) - query the status of a copy shard request
* [Kill Copy Shard](/enterprise_influxdb/v1.4/features/cluster-commands/#kill-copy-shard) - kill a running shard copy
* [Remove Shard](/enterprise_influxdb/v1.4/features/cluster-commands/#remove-shard) - remove a shard from a node (this deletes data)
* [Truncate Shards](/enterprise_influxdb/v1.4/features/cluster-commands/#truncate-shards) - truncate all active shard groups and start new shards immediately (This is useful when adding nodes or changing replication factors.)

This functionality is exposed via an API on the meta service and through [`influxd-ctl` sub-commands](/enterprise_influxdb/v1.4/features/cluster-commands/).

## OSS Conversion

Importing a OSS single server as the first data node is supported.

See [OSS to Cluster Migration](/enterprise_influxdb/v1.4/guides/migration/) for
...
version 0.7.1.
See [Backup and Restore](/enterprise_influxdb/v1.4/guides/backup-and-restore/) for
more information.
