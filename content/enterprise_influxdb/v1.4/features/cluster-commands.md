...
aliases:
    - /enterprise/v1.4/features/cluster-commands/
menu:
...
The `influxd-ctl` tool is available on all [meta nodes](/enterprise_influxdb/v1.4/concepts/glossary/#meta-node).
...
If authentication is enabled in the cluster's [meta node configuration files](/enterprise_influxdb/v1.4/administration/configuration/#auth-enabled-false) and [data node configuration files](/enterprise_influxdb/v1.4/administration/configuration/#meta-auth-enabled-false) and the `influxd-ctl` command does not include authentication details, the system returns:
```
...
Use `add-data` instead of the [`join` argument](#join) when performing a [Production Installation](/enterprise_influxdb/v1.4/production_installation/data_node_installation/) of an InfluxEnterprise cluster.

```
add-data <data-node-TCP-bind-address>
```

Resources: [Production Installation](/enterprise_influxdb/v1.4/production_installation/data_node_installation/)
...
Use `add-meta` instead of the [`join` argument](#join) when performing a [Production Installation](/enterprise_influxdb/v1.4/production_installation/meta_node_installation/) of an InfluxEnterprise cluster.
```
...
Creates a backup of a cluster's [metastore](/influxdb/v1.4/concepts/glossary/#metastore) and [shard](/influxdb/v1.4/concepts/glossary/#shard) data at that point in time and stores the copy in the specified directory.
...
`-full`:
&emsp;&emsp;&emsp;Perform a [full](/enterprise_influxdb/v1.4/guides/backup-and-restore/#backup) backup.

`-rp <retention-policy>`:
&emsp;&emsp;&emsp;The name of the single [retention policy](/influxdb/v1.4/concepts/glossary/#retention-policy-rp) to back up (requires the `-db` flag).
...
Resources: [Backup and Restore](/enterprise_influxdb/v1.4/guides/backup-and-restore/)
...
Copies a [shard](/influxdb/v1.4/concepts/glossary/#shard) from a source data node to a destination data node.
```
copy-shard <data-node-source-TCP-address> <data-node-destination-TCP-address> <shard-id>
```

Resources: [Cluster Rebalance](/enterprise_influxdb/v1.4/guides/rebalance/)
...
Shows all in-progress [copy shard](#copy-shard) operations, including the shard's source node, destination node, database, [retention policy](/influxdb/v1.4/concepts/glossary/#retention-policy-rp), shard id, total size, current size, and the operation's start time.
```
copy-shard-status
```
...
Use `join` instead of the [`add-meta`](#add-meta) or [`add-data`](#add-data) arguments when performing a [QuickStart Installation](/enterprise_influxdb/v1.4/quickstart_installation/cluster_installation/) of an InfluxEnterprise cluster.
```
...
Resources: [QuickStart Installation](/enterprise_influxdb/v1.4/quickstart_installation/cluster_installation/)
...
Use `leave` instead of the [`remove-meta`](#remove-meta) and [`remove-data`](#remove-data) arguments if you set up your InfluxEnterprise cluster with the [QuickStart Installation](/enterprise_influxdb/v1.4/quickstart_installation/cluster_installation/) process.
...
Use `remove-data` instead of the [`leave`](#leave) argument if you set up your InfluxEnterprise cluster with the [Production Installation](/enterprise_influxdb/v1.4/production_installation/) process.
...
Use `remove-meta` instead of the [`leave`](#leave) argument if you set up your InfluxEnterprise cluster with the [Production Installation](/enterprise_influxdb/v1.4/production_installation/) process.
...
`-newrf <int>`
&emsp;&emsp;&emsp;The new [replication factor](/influxdb/v1.4/concepts/glossary/#replication-factor) to restore to (this is capped to the number of data nodes in the cluster).

`-newrp <string>`
&emsp;&emsp;&emsp;The name of the new [retention policy](/influxdb/v1.4/concepts/glossary/#retention-policy-rp) to restore to (must specify with `-rp`).

`-rp <string>`
&emsp;&emsp;&emsp;The name of the single retention policy to restore.

`-shard <unit>`
&emsp;&emsp;&emsp;The [shard](/influxdb/v1.4/concepts/glossary/#shard) ID to restore.

Resources: [Backup and Restore](/enterprise_influxdb/v1.4/guides/backup-and-restore/#restore)
...
Shows all [meta nodes](/enterprise_influxdb/v1.4/concepts/glossary/#meta-node) and [data nodes](/enterprise_influxdb/v1.4/concepts/glossary/#data-node) that are part of the cluster.
...
Shows the cluster's existing [shards](/influxdb/v1.4/concepts/glossary/#shard), including the shard's id, database, [retention policy](/influxdb/v1.4/concepts/glossary/#retention-policy-rp), desired number of copies, [shard group](/influxdb/v1.4/concepts/glossary/#shard-group), start time, end time, expiry time and [data node](/enterprise_influxdb/v1.4/concepts/glossary/#data-node) owners.
```
show-shards
```
...
Updates a data node's address in the [meta store](/enterprise_influxdb/v1.4/concepts/glossary/#meta-service).
```
...
Truncates hot [shards](/influxdb/v1.4/concepts/glossary/#shard), that is, shards that cover the time range that includes the current time ([`now()`](/influxdb/v1.4/concepts/glossary/#now)).
...
`-delay <duration>`
&emsp;&emsp;&emsp;Determines when to truncate shards after [`now()`](/influxdb/v1.4/concepts/glossary/#now).
By default, the tool sets the delay to one minute.
The `duration` is an integer followed by a [duration unit](/influxdb/v1.4/query_language/spec/#durations).
...
The `influx` tool, also known as the Command Line Interface (CLI), is available on all [data nodes](/enterprise_influxdb/v1.4/concepts/glossary/#data-node).
Use `influx` to write data to your cluster, query data interactively, and view query output in different formats.

The complete description of the `influx` tool is available in the [OSS InfluxDB documentation](/influxdb/v1.4/tools/shell/).
