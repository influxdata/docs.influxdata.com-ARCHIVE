...
aliases:
    - /enterprise/v1.4/guides/rebalance/
menu:
...
* Evenly distribute
[shards](/influxdb/v1.4/concepts/glossary/#shard) across all data nodes in the
cluster
* Ensure that every
shard is on N number of nodes, where N is determined by the retention policy's
[replication factor](/influxdb/v1.4/concepts/glossary/#replication-factor)
...
[`influx-ctl` tool](/enterprise_influxdb/v1.4/features/cluster-commands/) available on
all meta nodes.
...
[replication factor](/influxdb/v1.4/concepts/glossary/#replication-factor) of
two.
...
[Update](/influxdb/v1.4/query_language/database_management/#modify-retention-policies-with-alter-retention-policy)
...
Here, we use InfluxDB's [CLI](/influxdb/v1.4/tools/shell/) to execute the query:
...
Use the
[`SHOW RETENTION POLICIES` query](/influxdb/v1.4/query_language/schema_exploration/#show-retention-policies)
