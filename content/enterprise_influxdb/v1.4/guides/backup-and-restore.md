...
aliases:
    - /enterprise/v1.4/guides/backup-and-restore/
menu:
...
single [shard](/influxdb/v1.4/concepts/glossary/#shard).

> **Note:** Backups are not interchangeable between [OSS InfluxDB](/influxdb/v1.4/) and InfluxEnterprise.
...
and Restore documentation](/influxdb/v1.4/administration/backup_and_restore/) in the
OSS InfluxDB documentation.

## Backup

A backup creates a copy of the [metastore](/influxdb/v1.4/concepts/glossary/#metastore) and [shard](/influxdb/v1.4/concepts/glossary/#shard) data at that point in time and stores the copy in the specified directory.
...
Please see the [influxd-ctl documentation](/enterprise_influxdb/v1.4/features/cluster-commands/#global-options)
...
By default, a restore writes to databases using the backed-up data's [replication factor](/influxdb/v1.4/concepts/glossary/#replication-factor).
...
[users](/influxdb/v1.4/concepts/glossary/#user), roles,
[databases](/influxdb/v1.4/concepts/glossary/#database), and
[continuous queries](/influxdb/v1.4/concepts/glossary/#continuous-query-cq) to
the new cluster.
>
They do not restore Kapacitor [subscriptions](/influxdb/v1.4/concepts/glossary/#subscription).
...
If [Telegraf](/telegraf/v1.4/) is still running, it will recreate the `telegraf` database shortly after the database is dropped.
...
Then, in the [`influx` client](/influxdb/v1.4/tools/shell/), use an [`INTO` query](/influxdb/v1.4/query_language/data_exploration/#relocate-data) to copy the data from the new database into the existing `telegraf` database:
