...
Use [`map()`](/flux/v0.64/stdlib/built-in/transformations/map/) to rename
...
Use the [`geo.s2CellIDToken()` function](/flux/v0.64/stdlib/experimental/geo/s2cellidtoken/)
with existing longitude (`lon`) and latitude (`lat`) field values to generate and add the S2 cell ID token.
First, use the [`geo.toRows()` function](/flux/v0.64/stdlib/experimental/geo/torows/)
