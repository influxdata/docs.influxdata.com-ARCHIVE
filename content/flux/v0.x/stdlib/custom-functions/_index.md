...
weight: 6
aliases:
  - /flux/v0.x/functions/custom-functions/
---
...
It uses the [`map()` function](/flux/v0.x/stdlib/built-in/transformations/map) to modify each `_value`.
...
It uses the [`sort()` function](/flux/v0.x/stdlib/built-in/transformations/sort) to sort records in either descending or ascending order.
It then uses the [`limit()` function](/flux/v0.x/stdlib/built-in/transformations/limit) to return the first record from the sorted table.
