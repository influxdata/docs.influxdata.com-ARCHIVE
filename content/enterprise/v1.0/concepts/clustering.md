...
The Enterprise Web Console serves the UI web application for managing and working with the InfluxDB cluster. It talks directly to the data and meta nodes over their HTTP protocols, which are bound by default to port `8086` for data nodes and `8088` for meta nodes. The Enterprise Web Console also needs to communicate over port `8091` to all meta nodes.
