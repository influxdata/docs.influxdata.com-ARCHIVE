...
* [Supported OAuth 2.0 authentication providers](#supported-oauth-2-0-authentication-providers)
...
* OAuth 2.0 authorization and authentication providers
* HTTPS support using TLS (Transport Layer Security)

The [OAuth 2.0 framework](https://tools.ietf.org/html/rfc6749) and supported authentication providers are used to provide the  authorization and authentication of Chronograph users and role-based access control.

The TLS (Transport Layer Security) cryptographic protocol is supported in Chronograf and provides server authentication, data confidentiality, and data integrity. Using TLS secures traffic between a server and web browser and enables the use of HTTPS.

## Supported OAuth 2.0 authentication providers
...
To use any of the supported OAuth authentication providers also requires the `TOKEN_SECRET` environment variable (or command line option) to generate valid JWT signatures.
The access tokens used by the OAuth providers are created using the [JSON Web Token (JWT)](https://tools.ietf.org/html/rfc7519) standard.

### `TOKEN_SECRET`

Chronograf uses [JSON Web Token (JWT)](https://tools.ietf.org/html/rfc7519) together with OAuth 2.0 authorization to provide authorization and authentication. As stated by the IETF:

"JSON Web Token (JWT) is a compact URL-safe means of representing claims to be transferred between two parties. The claims in a JWT are encoded as a JSON object that is digitally signed using JSON Web Signature (JWS)."

The required JWT signature is generated by the Chronograf service using a `TOKEN_SECRET` environment variable that you provide.

For the `TOKEN_SECRET`, provide a secure arbitrary string that Chronograf will use to generate the JWT Signature used with access tokens.
The `TOKEN_SECRET` is used every time the Chronograf server is started to verify user authorization.
To log all users out every time the server restarts, change the value of `TOKEN_SECRET` to a different value on each restart.
...
Chronograf has five environment variables (and corresponding command line options) for use with GitHub OAuth 2.0 authentication. The steps below show you how to get the required values for:

* TOKEN_SECRET (secret to use for generating JWT tokens)
* GH_CLIENT_ID (GitHub Client ID)
* GH_CLIENT_SECRET (GitHub Client Secret)

You can also specify values for two optional environment variables (or corresponding command line options):
...
The `Homepage URL` should include the full Chronograf server name and port. For example, if you are running it locally with the default settings, it would be `http://localhost:8888`.

The `Authorization callback URL` must be the location of the `Homepage URL` plus `/oauth/github/callback`.
For example, if `Homepage URL` was `http://localhost:8888`, then the `Authorization callback URL` would be `http://localhost:8888/oauth/github/callback`.
...
Chronograf supports the use of the Google OAuth 2.0 protocol for authentication and authorization. The steps below guide you in creating the following required Chronograf environment variables (or corresponding command line options):
...
This information must be set in the following environment variables (or corresponding command line options):

* `GOOGLE_CLIENT_ID` (Google Client ID)
* `GOOGLE_CLIENT_SECRET` (Google Client Secret)
* `PUBLIC_URL` (Public URL)

Additionally, you need to add the following required Chronograf environment variable (or corresponding command line option):
...
Click **Save** and then take note of your Domain, Client ID, and Client Secret at the top of the page.
...
The generic OAuth2 provider requires the following five environment variables:
...
The generic OAuth 2.0 provider has the following two optional environment variables:
...
Chronograf supports TLS (Transport Layer Security) to enable secure communications between web browsers and servers using HTTPS.

InfluxData recommends using HTTPS with Chronograf.
If you are not using a TLS termination proxy, you can run Chronograf's server with TLS connections.

### Using HTTPS and TLS with Chronograf
