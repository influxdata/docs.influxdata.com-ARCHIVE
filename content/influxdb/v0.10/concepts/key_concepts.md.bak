...
[Queries](/influxdb/v0.9/concepts/glossary/#query) that use field values as filters must scan all values that match the other conditions in the query.
...
To optimize your queries, it may be beneficial to rearrange your [schema](/influxdb/v0.9/concepts/glossary/#schema) such that the fields (`butterflies` and `honeybees`) become the tags and the tags (`location` and `scientist`) become the fields:
...
See [users](/influxdb/v0.9/administration/authentication_and_authorization/) and [continuous queries](/influxdb/v0.9/query_language/continuous_queries/) for more on those topics.
...
If you're just starting out, we recommend taking a look at [Getting Started](/influxdb/v0.9/introduction/getting_started/) and the [Writing Data](/influxdb/v0.9/guides/writing_data/) and [Querying Data](/influxdb/v0.9/guides/querying_data/) guides.
