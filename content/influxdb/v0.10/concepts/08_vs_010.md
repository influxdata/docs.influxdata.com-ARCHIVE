...
In InfluxDB 0.10, fields can be passed into functions but tags cannot.
Tags can be used in a `GROUP BY` clause but fields cannot.
(In InfluxDB 0.8 series values -- equivalent to fields -- can be used in the `GROUP BY`  clause.) In InfluxDB 0.10, measurement names and tags can be matched using regular expressions but regular expressions are not valid for matching field keys or values.
...
In InfluxDB 0.10 a point is uniquely identified by the measurement name, full tag set, and the timestamp.
