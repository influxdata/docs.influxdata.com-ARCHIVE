...
There are many ways to write data into InfluxDB including the [command line interface](/influxdb/v0.10/tools/shell/), [client libraries](/influxdb/v0.10/clients/api/) and plugins for common data formats such as [Graphite](/influxdb/v0.10/write_protocols/graphite/).
...
See the [HTTP section](/influxdb/v0.10/write_protocols/write_syntax/#http) on the Write Syntax page for a complete list of the available query parameters.

The body of the POST - we call this the [Line Protocol](/influxdb/v0.10/write_protocols/line/) - contains the time-series data that you wish to store.
...
Field keys are required and are always strings, and, [by default](/influxdb/v0.10/write_protocols/write_syntax/#line-protocol), field values are floats.
...
The data in the file should follow InfluxDB's [line protocol syntax](/influxdb/v0.10/write_protocols/write_syntax/).
...
[By default](/influxdb/v0.10/administration/config/#cluster), the HTTP request times out after five seconds.
...
This simple, easy to understand system uses HTTP as a transfer system for [InfluxQL](https://github.com/influxdb/influxdb/blob/master/influxql/INFLUXQL.md).
...
### Next steps
---
Now that you know how to write data with the built-in HTTP API discover how to query them with the [Querying Data](/influxdb/v0.10/guides/querying_data/) guide!
