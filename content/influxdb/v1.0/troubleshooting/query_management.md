...
<br>
`qid`: The id number of the query. Use this value with [`KILL QUERY`](/influxdb/v1.0/troubleshooting/query_management/#stop-currently-running-queries-with-kill-query).  
`query`: The query text.  
`database`: The database targeted by the query.  
`duration`: The length of time that the query has been running.
See [Query Language Reference](/influxdb/v1.0/query_language/spec/#durations)
...
##### Syntax:
Where `qid` is the id of the query from the [`SHOW QUERIES`](/influxdb/v1.0/troubleshooting/query_management/#list-currently-running-queries-with-show-queries) output:
```
KILL QUERY <qid>
```
...
Use this value with [`KILL QUERY`](/influxdb/v1.0/troubleshooting/query_management/#stop-currently-running-queries-with-kill-query).
