...
The time ranges of the CQ results have natural time boundaries that are set internally by the database. There is currently no way for users to alter the start or end times of the intervals.
...
If you attempt to create a continuous query that already exists and the body of the CQ has chaned, InfluxDB will return the error `continuous query already exists`.
