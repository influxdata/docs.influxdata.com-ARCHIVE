...
Note that the query results are identical to the results of the query in [Example 2](#group-query-results-by-more-than-one-tag)
...
The [backreference](#write-aggregated-results-for-more-than-one-measurement-to-a-different-database-downsampling-with-backreferencing) syntax (`:MEASUREMENT`) maintains the source measurement names in the destination database.
...
The query uses an InfluxQL [function](/influxdb/v1.7/query_language/functions/)
to return the distinct [field values](/influxdb/v1.7/concepts/glossary/#field-value)
