...
Retention policy auto-creation automatically creates the [`DEFAULT` retention policy](/influxdb/v1.7/concepts/glossary/#retention-policy-rp) `autogen` when a database is created.
...
The WAL directory is the location of the [write ahead log](/influxdb/v1.7/concepts/glossary/#wal-write-ahead-log).
...
Determines whether HTTP endpoint is enabled. To disable HTTP, set the value to `false`. Note that the InfluxDB [command line interface (CLI)](/influxdb/v1.7/tools/shell/) connects to the database using the HTTP API.
...
Limits the number of rows that the system can return in a [non-chunked](/influxdb/v1.7/tools/api/#query-string-parameters) query.
