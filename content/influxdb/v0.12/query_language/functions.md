...
* [Include multiple functions in a single query](/influxdb/v0.12/query_language/functions/#include-multiple-functions-in-a-single-query)
* [Change the value reported for intervals with no data with `fill()` ](/influxdb/v0.12/query_language/functions/#change-the-value-reported-for-intervals-with-no-data-with-fill)
* [Rename the output column's title with `AS`](/influxdb/v0.12/query_language/functions/#rename-the-output-column-s-title-with-as)
...
## COUNT()
Returns the number of non-null values in a single [field](/influxdb/v0.12/concepts/glossary/#field).
```sql
...
> For a more general discussion of `fill()`, see [Data Exploration](/influxdb/v0.12/query_language/data_exploration/#the-group-by-clause-and-fill).

## DISTINCT()
Returns the unique values of a single [field](/influxdb/v0.12/concepts/glossary/#field).
```sql
...
* Nest `DISTINCT()` in [`COUNT()`](/influxdb/v0.12/query_language/functions/#count) to get the number of unique field values in `level description` grouped by the `location` tag:
...
## MEDIAN()
Returns the middle value from the sorted values in a single [field](/influxdb/v0.12/concepts/glossary/#field).
...
> **Note:** `MEDIAN()` is nearly equivalent to [`PERCENTILE(field_key, 50)`](/influxdb/v0.12/query_language/functions/#percentile), except `MEDIAN()` returns the average of the two middle values if the field contains an even number of points.
...
## SUM()
Returns the sum of the all values in a single [field](/influxdb/v0.12/concepts/glossary/#field).
The field must be of type int64 or float64.
```sql
...
## BOTTOM()
Returns the smallest `N` values in a single [field](/influxdb/v0.12/concepts/glossary/#field).
The field type must be int64 or float64.
```sql
...
## PERCENTILE()
Returns the `N`th percentile value for the sorted values of a single [field](/influxdb/v0.12/concepts/glossary/#field).
The field must be of type int64 or float64.
...
## TOP()
Returns the largest `N` values in a single [field](/influxdb/v0.12/concepts/glossary/#field).
The field type must be int64 or float64.
```sql
...
## DIFFERENCE()
Returns the difference between consecutive chronological values in a single [field](/influxdb/v0.12/concepts/glossary/#field).
The field type must be int64 or float64.
...
See [`DERIVATIVE()`](/influxdb/v0.12/query_language/functions/#derivative) for example queries.
...
## STDDEV()
Returns the standard deviation of the values in a single [field](/influxdb/v0.12/concepts/glossary/#field).
The field must be of type int64 or float64.
```sql
...
Calculate the [minimum](/influxdb/v0.12/query_language/functions/#min) `water_level` and the [maximum](/influxdb/v0.12/query_language/functions/#max) `water_level` with a single query:
```sql
...
For a complete discussion of `fill()`, see [Data Exploration](/influxdb/v0.12/query_language/data_exploration/#the-group-by-clause-and-fill).

> **Note:** `fill()` works differently with `COUNT()`.
See [the documentation on `COUNT()`](/influxdb/v0.12/query_language/functions/#count-and-controlling-the-values-reported-for-intervals-with-no-data) for a function-specific use of `fill()`.
