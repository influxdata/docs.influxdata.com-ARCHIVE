...
* [The `SELECT` statement and the `WHERE` clause](/influxdb/v0.12/query_language/data_exploration/#the-select-statement-and-the-where-clause)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[The basic `SELECT` statement](/influxdb/v0.12/query_language/data_exploration/#the-basic-select-statement)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[The `SELECT` statement and arithmetic](/influxdb/v0.12/query_language/data_exploration/#the-select-statement-and-arithmetic)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[The `WHERE` clause](/influxdb/v0.12/query_language/data_exploration/#the-where-clause)
* [The `GROUP BY` clause](/influxdb/v0.12/query_language/data_exploration/#the-group-by-clause)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[The basic `GROUP BY` clause](/influxdb/v0.12/query_language/data_exploration/#the-basic-group-by-clause)   
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[The `GROUP BY` clause and `fill()`](/influxdb/v0.12/query_language/data_exploration/#the-group-by-clause-and-fill)  
* [The `INTO` clause](/influxdb/v0.12/query_language/data_exploration/#the-into-clause)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Relocate data](/influxdb/v0.12/query_language/data_exploration/#relocate-data)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Downsample data](/influxdb/v0.12/query_language/data_exploration/#downsample-data)    

Limit and sort your results:

* [Limit query returns with `LIMIT` and `SLIMIT`](/influxdb/v0.12/query_language/data_exploration/#limit-query-returns-with-limit-and-slimit)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Limit results per series with `LIMIT`](/influxdb/v0.12/query_language/data_exploration/#limit-the-number-of-results-returned-per-series-with-limit)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Limit the number of series returned with `SLIMIT`](/influxdb/v0.12/query_language/data_exploration/#limit-the-number-of-series-returned-with-slimit)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Limit the number of points and series returned with `LIMIT` and `SLIMIT`](/influxdb/v0.12/query_language/data_exploration/#limit-the-number-of-points-and-series-returned-with-limit-and-slimit)
* [Sort query returns with `ORDER BY time DESC`](/influxdb/v0.12/query_language/data_exploration/#sort-query-returns-with-order-by-time-desc)
* [Paginate query returns with `OFFSET`](/influxdb/v0.12/query_language/data_exploration/#paginate-query-returns-with-offset)

General tips on query syntax:

* [Multiple statements in queries](/influxdb/v0.12/query_language/data_exploration/#multiple-statements-in-queries)
* [Merge series in queries](/influxdb/v0.12/query_language/data_exploration/#merge-series-in-queries)
* [Time syntax in queries](/influxdb/v0.12/query_language/data_exploration/#time-syntax-in-queries)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Relative time](/influxdb/v0.12/query_language/data_exploration/#relative-time)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Absolute time](/influxdb/v0.12/query_language/data_exploration/#absolute-time)
* [Regular expressions in queries](/influxdb/v0.12/query_language/data_exploration/#regular-expressions-in-queries)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Regular expressions and selecting measurements](/influxdb/v0.12/query_language/data_exploration/#regular-expressions-and-selecting-measurements)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Regular expressions and specifying tags](/influxdb/v0.12/query_language/data_exploration/#regular-expressions-and-specifying-tags)

The examples below query data using [InfluxDB's Command Line Interface (CLI)](/influxdb/v0.12/tools/shell/).
See the [Querying Data](/influxdb/v0.12/guides/querying_data/) guide for how to query data directly using the HTTP API.

#### Sample data
<br>
If you'd like to follow along with the queries in this document, see [Sample Data](/influxdb/v0.12/sample_data/data_download/) for how to download and write the data to InfluxDB.
...
Identifiers are database names, retention policy names, user names, measurement names, tag keys, and field keys.
 ...
 ```
* Fully qualify a measurement if you wish to query data from a different database or from a retention policy other than the default [retention policy](/influxdb/v0.12/concepts/glossary/#retention-policy-rp).
...
This can lead to [overflow issues](/influxdb/v0.12/troubleshooting/frequently_encountered_issues/#working-with-really-big-or-really-small-integers) for some numbers.

### The `WHERE` clause
---
Use a `WHERE` clause to filter your data based on tags, time ranges, and/or field values.

> **Note:** The quoting syntax for queries differs from the line protocol. 
Please review the [rules for single and double-quoting](https://docs.influxdata.com/influxdb/v0.12/troubleshooting/frequently_encountered_issues/#single-quoting-and-double-quoting-in-queries) in queries.
...
Return data where the tag key `location` has no tag value (more on regular expressions [later](/influxdb/v0.12/query_language/data_exploration/#regular-expressions-in-queries)):
```sql
...
For more on `now()` and other ways to specify time in queries, see [time syntax in queries](/influxdb/v0.12/query_language/data_exploration/#time-syntax-in-queries).
...
To successfully implement `GROUP BY`,  append the`GROUP BY` clause to a `SELECT` statement and pair the `SELECT` statement with one of InfluxQL's [functions](/influxdb/v0.12/query_language/functions/).
...
---
**GROUP BY tag values**  
Calculate the [`MEAN()`](/influxdb/v0.12/query_language/functions/#mean) `water_level` for the different tag values of `location`:
```sql
...
Calculate the [`MEAN()`](/influxdb/v0.12/query_language/functions/#mean) `index` for every tag set in `h2o_quality`:
```sql
...
[`COUNT()`](/influxdb/v0.12/query_language/functions/#count) the number of `water_level` points between August 18, 2015 at midnight and September 18 at 5:00pm at two day intervals:
```sql
...
See [Frequently Encountered Issues](/influxdb/v0.12/troubleshooting/frequently_encountered_issues/#understanding-the-time-intervals-returned-from-group-by-time-queries) for an explanation of the time intervals returned from `GROUP BY time()` queries.
...
Write the field `water_level` in `h2o_feet` to a new measurement (`h2o_feet_copy`) and to the retention policy `default` in the [already-existing](/influxdb/v0.12/query_language/database_management/#create-a-database-with-create-database) database `where_else`:
```sql
...
### Downsample data
Combine the `INTO` clause with an InfluxQL [function](/influxdb/v0.12/query_language/functions/) and a `GROUP BY` clause to write the lower precision query results to a different measurement:
```sql
...
If you want InfluxDB to automatically query and downsample all future data see [Continuous Queries](/influxdb/v0.12/query_language/continuous_queries/).
...
Use [fill()](/influxdb/v0.12/query_language/data_exploration/#the-group-by-clause-and-fill) to ensure that every time interval appears in the results.
...
* `LIMIT <N>` returns the first \<N> [points](/influxdb/v0.12/concepts/glossary/#point) from each [series](/influxdb/v0.12/concepts/glossary/#series) in the specified measurement.
...
The `NOAA_water_database` database has two [series](/influxdb/v0.12/concepts/glossary/#series).
...
The following query automatically merges those two series when it calculates the [average](/influxdb/v0.12/query_language/functions/#mean) `water_level`:
...
In this section we'll be using all of the measurements in the [sample data](/influxdb/v0.12/query_language/data_exploration/#sample-data):
...
See [the WHERE clause](/influxdb/v0.12/query_language/data_exploration/#the-where-clause) section for an example of how to return data where a tag key has a value and an example of how to return data where a tag key has no value using regular expressions.
