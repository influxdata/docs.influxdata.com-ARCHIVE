...
See [Database Management](/influxdb/v0.12/query_language/database_management/#retention-policy-management) for more on RP management.
...
See [Continuous Queries](/influxdb/v0.12/query_language/continuous_queries/) for a detailed discussion on how to create and manage CQs.
...
The following steps work with a fictional [database](/influxdb/v0.12/concepts/glossary/#database) called `food_data` and the [measurement](/influxdb/v0.12/concepts/glossary/#measurement) `orders`.
`orders` has two [fields](/influxdb/v0.12/concepts/glossary/#field), `phone` and `website`, which store the number of orders that arrive via each channel every 10 seconds.
...
For a more detailed discussion on the `CREATE RETENTION POLICY` syntax, see [Database Management](/influxdb/v0.12/query_language/database_management/#retention-policy-management).

To clarify, we've included the results from the [`SHOW RETENTION POLICIES`](/influxdb/v0.12/query_language/schema_exploration/#explore-retention-policies-with-show-retention-policies) query below.
...
For a more detailed discussion on the `CREATE CONTINUOUS QUERY` syntax, see [Continuous Queries](/influxdb/v0.12/query_language/continuous_queries/).
...
The rate at which InfluxDB checks to enforce an RP is a configurable setting, see [Database Configuration](/influxdb/v0.12/administration/config/#retention).
