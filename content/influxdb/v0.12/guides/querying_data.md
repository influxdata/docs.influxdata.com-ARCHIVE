...
The HTTP API is the primary means for querying data in InfluxDB (see the [command line interface](/influxdb/v0.12/tools/shell/) and [client libraries](/influxdb/v0.12/clients/api/) for alternative ways to query the database).
...
The example below uses the HTTP API to query the same database that you encountered in [Writing Data](/influxdb/v0.12/guides/writing_data/).
<br>
```bash
...
See [Authentication and Authorization](/influxdb/v0.12/administration/authentication_and_authorization/) for how to enable and set up authentication.
...
### InfluxQL
---
Now that you know how to query data, check out the [Data Exploration page](/influxdb/v0.12/query_language/data_exploration/) to get acquainted with InfluxQL.
