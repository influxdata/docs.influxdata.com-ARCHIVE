...
* [Understanding the time intervals returned from `GROUP BY time()` queries](/influxdb/v0.12/troubleshooting/frequently_encountered_issues/#understanding-the-time-intervals-returned-from-group-by-time-queries)    
* [Querying after `now()`](/influxdb/v0.12/troubleshooting/frequently_encountered_issues/#querying-after-now)  
...
* [Identifying write precision from returned timestamps](/influxdb/v0.12/troubleshooting/frequently_encountered_issues/#identifying-write-precision-from-returned-timestamps)  
* [Single quoting and double quoting in queries](/influxdb/v0.12/troubleshooting/frequently_encountered_issues/#single-quoting-and-double-quoting-in-queries)  
* [Missing data after creating a new `DEFAULT` retention policy](/influxdb/v0.12/troubleshooting/frequently_encountered_issues/#missing-data-after-creating-a-new-default-retention-policy)
...
* [Single quoting and double quoting when writing data](/influxdb/v0.12/troubleshooting/frequently_encountered_issues/#single-quoting-and-double-quoting-when-writing-data)  

**Administration**  

* [Single quoting the password string](/influxdb/v0.12/troubleshooting/frequently_encountered_issues/#single-quoting-the-password-string)
* [Escaping the single quote in a password](/influxdb/v0.12/troubleshooting/frequently_encountered_issues/#escaping-the-single-quote-in-a-password)  
* [Identifying your version of InfluxDB](/influxdb/v0.12/troubleshooting/frequently_encountered_issues/#identifying-your-version-of-influxdb)  
* [Data aren't dropped after altering a retention policy](/influxdb/v0.12/troubleshooting/frequently_encountered_issues/#data-aren-t-dropped-after-altering-a-retention-policy)
...
In the example below the first [timestamp](/influxdb/v0.12/concepts/glossary/#timestamp) in the results occurs before the lower bound of the query:
...
```
We [create](/influxdb/v0.12/query_language/database_management/#create-retention-policies-with-create-retention-policy) a new `DEFAULT` RP (`two_hour`) and perform the same query:
```bash
> SELECT count(flounders) FROM fleeting
>
```
...
In InfluxDB 0.11 a point is uniquely identified by the measurement name, [tag set](/influxdb/v0.12/concepts/glossary/#tag-set), and the nanosecond timestamp.
...
First, double check your [line protocol](/influxdb/v0.12/write_protocols/line/) syntax.
...
See the [Line Protocol Syntax](/influxdb/v0.12/write_protocols/write_syntax/) page for more information.
...
* Check the text that appears when you [launch](/influxdb/v0.12/tools/shell/) the CLI:
...
We do this by [altering](/influxdb/v0.12/query_language/database_management/#modify-retention-policies-with-alter-retention-policy) the retention policy:
<br>
<br>
```
...
At the next [retention policy enforcement check](/influxdb/v0.12/administration/config/#retention), InfluxDB immediately drops `Shard Group 1` because all of its data are older than 3 days.
...
See [Database Configuration](/influxdb/v0.12/administration/config/#shard-precreation) for more on that setting.
See [Database Management](/influxdb/v0.12/query_language/database_management/#delete-a-shard-with-drop-shard) for how to delete a shard.
