...
* [Launch `influx`](/influxdb/v0.12/tools/shell/#launch-influx)
* [`influx` Arguments](/influxdb/v0.12/tools/shell/#influx-arguments)
...
```bash
$ influx
Connected to http://localhost:8086 version 0.12.x
InfluxDB shell 0.12.x
```
...
`-execute 'command'`  
Execute an [InfluxQL](/influxdb/v0.12/query_language/data_exploration/) command and quit.
See [-execute](/influxdb/v0.12/tools/shell/#execute-an-influxql-command-and-quit-with-execute).

`-format 'json|csv|column'`  
Specifies the format of the server responses.
See [-format](/influxdb/v0.12/tools/shell/#specify-the-format-of-the-server-responses-with-format).
...
See [-import](/influxdb/v0.12/tools/shell/#import-data-from-a-file-with-import).
...
* **DDL (Data Definition Language)**: Contains the [InfluxQL commands](/influxdb/v0.12/query_language/database_management/) for creating the relevant [database](/influxdb/v0.12/concepts/glossary/) and managing the [retention policy](/influxdb/v0.12/concepts/glossary/#retention-policy-rp).
If your database and retention policy already exist, your file can skip this section.
* **DML (Data Manipulation Language)**: Lists the relevant database and (if desired) retention policy and contains the data in [line protocol](/influxdb/v0.12/write_protocols/line/).
...
See the description of [-format](/influxdb/v0.12/tools/shell/#specify-the-format-of-the-server-responses-with-format) for examples of each format.
...
See [insert](/influxdb/v0.12/tools/shell/#write-data-to-influxdb-with-insert).
...
Enter `insert` followed by the data in [line protocol](/influxdb/v0.12/write_protocols/line/) to write data to InfluxDB.
Use `insert into <retention policy> <line protocol>` to write data to a specific [retention policy](/influxdb/v0.12/concepts/glossary/#retention-policy-rp).
...
See [Data Exploration](/influxdb/v0.12/query_language/data_exploration/), [Schema Exploration](/influxdb/v0.12/query_language/schema_exploration/), [Database Management](/influxdb/v0.12/query_language/database_management/), [Authentication and Authorization](/influxdb/v0.12/administration/authentication_and_authorization/) for InfluxQL documentation.
