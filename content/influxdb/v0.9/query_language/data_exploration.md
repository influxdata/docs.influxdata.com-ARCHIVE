...
The [series](/influxdb/v0.9/concepts/glossary/#series) are made up of the [measurement](/influxdb/v0.9/concepts/glossary/#measurement) `h2o_feet` and the [tag key](/influxdb/v0.9/concepts/glossary/#tag-key) `location` with the [tag values](/influxdb/v0.9/concepts/glossary/#tag-value) `santa_monica` and `coyote_creek`.
There are two [fields](/influxdb/v0.9/concepts/glossary/#field): `water_level` which stores floats and `level description` which stores strings.
...
Identifiers are database names, retention policy names, user names, measurement names, tag keys, and field keys.
 ...
 ```
* Fully qualify a measurement if you wish to query data from a different database or from a retention policy other than the default [retention policy](/influxdb/v0.9/concepts/glossary/#retention-policy).
...
This can lead to [overflow issues](/influxdb/v0.9/troubleshooting/frequently_encountered_issues/#working-with-really-big-or-really-small-integers) for some numbers.
...
For more on `now()` and other ways to specify time in queries, see [time syntax in queries](/influxdb/v0.9/query_language/data_exploration/#time-syntax-in-queries).
...
To successfully implement `GROUP BY`,  append the`GROUP BY` clause to a `SELECT` statement and pair the `SELECT` statement with one of InfluxQL's [functions](/influxdb/v0.9/query_language/functions/).
...
---
**GROUP BY tag values**  
Calculate the [`MEAN()`]/influxdb/v0.9/query_language/functions/#mean) `water_level` for the different tag values of `location`:
```sql
...
Calculate the [`MEAN()`]/influxdb/v0.9/query_language/functions/#mean) `index` for every tag set in `h2o_quality`:
```sql
...
[`COUNT()`]/influxdb/v0.9/query_language/functions/#count) the number of `water_level` points between August 18, 2015 at midnight and September 18 at 5:00pm at two day intervals:
```sql
...
See [Frequently Encountered Issues](/influxdb/v0.9/troubleshooting/frequently_encountered_issues/#understanding-the-time-intervals-returned-from-group-by-time-queries) for an explanation of the time intervals returned from `GROUP BY time()` queries.
...
If you want InfluxDB to automatically query and downsample all future data see [Continuous Queries](/influxdb/v0.9/query_language/continuous_queries/).
...
Use [fill()](/influxdb/v0.9/query_language/data_exploration/#the-group-by-clause-and-fill) to ensure that every time interval appears in the results.
...
The `NOAA_water_database` database has two [series](/influxdb/v0.9/concepts/glossary/#series).
...
The following query automatically merges those two series when it calculates the [`MEAN()`]/influxdb/v0.9/query_language/functions/#mean) `water_level`:
...
The [sample data](/influxdb/v0.9/query_language/data_exploration/#sample-data) need to be more intricate for the following sections.
