...
* [Launch `influx`](/influxdb/v1.1/tools/shell/#launch-influx)
* [`influx` Arguments](/influxdb/v1.1/tools/shell/#influx-arguments)
...
`-execute 'command'`  
Execute an [InfluxQL](/influxdb/v1.1/query_language/data_exploration/) command and quit.
See [-execute](/influxdb/v1.1/tools/shell/#execute-an-influxql-command-and-quit-with-execute).

`-format 'json|csv|column'`  
Specifies the format of the server responses.
See [-format](/influxdb/v1.1/tools/shell/#specify-the-format-of-the-server-responses-with-format).
...
`-import`  
Import new data from a file or import a previously [exported](https://github.com/influxdb/influxdb/blob/1.1/importer/README.md) database from a file.
See [-import](/influxdb/v1.1/tools/shell/#import-data-from-a-file-with-import).
...
* **DDL (Data Definition Language)**: Contains the [InfluxQL commands](/influxdb/v1.1/query_language/database_management/) for creating the relevant [database](/influxdb/v1.1/concepts/glossary/) and managing the [retention policy](/influxdb/v1.1/concepts/glossary/#retention-policy-rp).
If your database and retention policy already exist, your file can skip this section.
* **DML (Data Manipulation Language)**: Lists the relevant database and (if desired) retention policy and contains the data in [line protocol](/influxdb/v1.1/concepts/glossary/#line-protocol).
...
* Include timestamps in the data file. InfluxDB will assign the same timestamp to points without a timestamp. This can lead to unintended [overwrite behavior](/influxdb/v1.1/troubleshooting/frequently-asked-questions/#how-does-influxdb-handle-duplicate-points).
...
> **Note:** For how to export data from InfluxDB version 0.8.9, see [Exporting from 0.8.9](https://github.com/influxdb/influxdb/blob/1.1/importer/README.md).
...
See the description of [-format](/influxdb/v1.1/tools/shell/#specify-the-format-of-the-server-responses-with-format) for examples of each format.
...
See [insert](/influxdb/v1.1/tools/shell/#write-data-to-influxdb-with-insert).
...
Use `insert into <retention policy> <line protocol>` to write data to a specific [retention policy](/influxdb/v1.1/concepts/glossary/#retention-policy-rp).
...
See [Data Exploration](/influxdb/v1.1/query_language/data_exploration/), [Schema Exploration](/influxdb/v1.1/query_language/schema_exploration/), [Database Management](/influxdb/v1.1/query_language/database_management/), [Authentication and Authorization](/influxdb/v1.1/query_language/authentication_and_authorization/) for InfluxQL documentation.
