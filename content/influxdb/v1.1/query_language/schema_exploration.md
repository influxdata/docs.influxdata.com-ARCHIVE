...
* [See all databases with `SHOW DATABASES`](/influxdb/v1.1/query_language/schema_exploration/#see-all-databases-with-show-databases)
* [Explore retention policies with `SHOW RETENTION POLICIES`](/influxdb/v1.1/query_language/schema_exploration/#explore-retention-policies-with-show-retention-policies)
* [Explore series with `SHOW SERIES`](/influxdb/v1.1/query_language/schema_exploration/#explore-series-with-show-series)
* [Explore measurements with `SHOW MEASUREMENTS`](/influxdb/v1.1/query_language/schema_exploration/#explore-measurements-with-show-measurements)
* [Explore tag keys with `SHOW TAG KEYS`](/influxdb/v1.1/query_language/schema_exploration/#explore-tag-keys-with-show-tag-keys)
* [Explore tag values with `SHOW TAG VALUES`](/influxdb/v1.1/query_language/schema_exploration/#explore-tag-values-with-show-tag-values)
* [Explore field keys with `SHOW FIELD KEYS`](/influxdb/v1.1/query_language/schema_exploration/#explore-field-keys-with-show-field-keys)

The examples below query data using [InfluxDB's Command Line Interface (CLI)](/influxdb/v1.1/tools/shell/).
See the [Querying Data](/influxdb/v1.1/guides/querying_data/) guide for how to directly query data with the HTTP API.

**Sample data**

This document uses the same sample data as the [Data Exploration](/influxdb/v1.1/query_language/data_exploration/) page.
...
The `SHOW RETENTION POLICIES` query lists the existing [retention policies](/influxdb/v1.1/concepts/glossary/#retention-policy-rp) on a given database, and it takes the following form:
```sql
...
The second column shows the retention policy's [duration](/influxdb/v1.1/concepts/glossary/#duration) and the third column shows the [shard group](/influxdb/v1.1/concepts/glossary/#shard-group) duration. The fourth column shows the [replication factor](/influxdb/v1.1/concepts/glossary/#replication-factor) of the retention policy and the fifth column specifies if the retention policy is the `DEFAULT` retention policy for the database.
...
The `SHOW SERIES` query returns the distinct [series](/influxdb/v1.1/concepts/glossary/#series) in your database and takes the following form, where the `FROM` and `WHERE` clauses are optional:
...
`SHOW SERIES` organizes its output similar to the [line protocol](/influxdb/v1.1/concepts/glossary/#line-protocol) format.
...
The `SHOW MEASUREMENTS` query returns the [measurements](/influxdb/v1.1/concepts/glossary/#measurement) in your database and it takes the following form:
```sql
...
`SHOW TAG KEYS` returns the [tag keys](/influxdb/v1.1/concepts/glossary/#tag-key) associated with each measurement and takes the following form, where the `FROM` clause is optional:
```sql
...
The `SHOW TAG VALUES` query returns the set of [tag values](/influxdb/v1.1/concepts/glossary/#tag-value) for a specific tag key across all measurements in the database.
```sql
...
The `SHOW FIELD KEYS` query returns the [field keys](/influxdb/v1.1/concepts/glossary/#field-key)
and [field value](/influxdb/v1.1/concepts/glossary/#field-value) data [types](/influxdb/v1.1/write_protocols/write_syntax/#data-types) across each measurement in the database.
