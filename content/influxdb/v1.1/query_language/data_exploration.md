...
See the [Sample Data](/influxdb/v1.1/query_language/data_download/) page to download
...
The data in the `h2o_feet` [measurement](/influxdb/v1.1/concepts/glossary/#measurement)
occur at six-minute time intervals.
The measurement has one [tag key](/influxdb/v1.1/concepts/glossary/#tag-key)
(`location`) which has two [tag values](/influxdb/v1.1/concepts/glossary/#tag-value):
`coyote_creek` and `santa_monica`.
The measurement also has two [fields](/influxdb/v1.1/concepts/glossary/#field):
`level description` stores string [field values](/influxdb/v1.1/concepts/glossary/#field-value)
and `water_level` stores float field values.
All of these data are in the `NOAA_water_database` [database](/influxdb/v1.1/concepts/glossary/#database).
...
`SELECT *`  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Returns all [fields](/influxdb/v1.1/concepts/glossary/#field) and [tags](/influxdb/v1.1/concepts/glossary/#tag).
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Returns a specific field and tag.
The `::[field | tag]` syntax specifies the [identifier's](/influxdb/v1.1/concepts/glossary/#identifier) type.
Use this syntax to differentiate between field keys and tag keys that have the same name.

Other supported features:
[Arithmetic Operations](/influxdb/v1.1/query_language/math_operators/),
...
#### `FROM` clause
The `FROM` clause supports several formats for specifying a [measurement(s)](/influxdb/v1.1/concepts/glossary/#measurement):

`FROM <measurement_name>`  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
Returns data from a single measurement.
If you're using the [CLI](/influxdb/v1.1/tools/shell/) InfluxDB queries the measurement in the
[`USE`d](/influxdb/v1.1/tools/shell/#commands)
[database](/influxdb/v1.1/concepts/glossary/#database) and the `DEFAULT` [retention policy](/influxdb/v1.1/concepts/glossary/#retention-policy-rp).
If you're using the [HTTP API](/influxdb/v1.1/tools/api/) InfluxDB queries the
measurement in the database specified in the [`db` query string parameter](/influxdb/v1.1/tools/api/#query-string-parameters)
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
Returns data from a measurement in a user-specified [database](/influxdb/v1.1/concepts/glossary/#database) and the `DEFAULT`
...
Please review the [rules for single and double-quoting](/influxdb/v1.1/troubleshooting/frequently-asked-questions/#when-should-i-single-quote-and-when-should-i-double-quote-in-queries) in queries.
...
The query selects all [fields](/influxdb/v1.1/concepts/glossary/#field) and
[tags](/influxdb/v1.1/concepts/glossary/#tag) from the `h2o_feet`
[measurement](/influxdb/v1.1/concepts/glossary/#measurement).

If you're using the [CLI](/influxdb/v1.1/tools/shell/) be sure to enter
`USE NOAA_water_database` before you run the query.
The CLI queries the data in the `USE`d database and the
`DEFAULT` [retention policy](/influxdb/v1.1/concepts/glossary/#retention-policy-rp).
If you're using the [HTTP API](/influxdb/v1.1/tools/api/) be sure to set the
`db` [query string parameter](/influxdb/v1.1/tools/api/#query-string-parameters)
...
[identifier](/influxdb/v1.1/concepts/glossary/#identifier) is a field or tag.
Use `::[field | tag]` to differentiate between [an identical field key and tag key ](/influxdb/v1.1/troubleshooting/frequently-asked-questions/#how-do-i-query-data-with-an-identical-tag-key-and-field-key).
...
See [Mathematical Operators](/influxdb/v1.1/query_language/math_operators/)
...
and integer [field values](/influxdb/v1.1/concepts/glossary/#field-value).
...
Single quote [tag values](/influxdb/v1.1/concepts/glossary/#tag-value) in
the `WHERE` clause.
...
[measurement](/influxdb/v1.1/concepts/glossary/#measurement) with
[field values](/influxdb/v1.1/concepts/glossary/#field-value) of `water_level`
...
[tag key](/influxdb/v1.1/concepts/glossary/#tag-key) `location` is set to `santa_monica`.
...
The query returns data from the `h2o_feet` measurement that have [timestamps](/influxdb/v1.1/concepts/glossary/#timestamp)
...
or string [field values](/influxdb/v1.1/concepts/glossary/#field-value).
...
set of [tags](/influxdb/v1.1/concepts/glossary/#tag) or a time interval.
...
`GROUP BY <tag>` queries group query results by a user-specified set of [tags](/influxdb/v1.1/concepts/glossary/#tag).
...
`GROUP BY *`  
&emsp;&emsp;&emsp;Groups results by all [tags](/influxdb/v1.1/concepts/glossary/#tag)
...
The order of the [tag keys](/influxdb/v1.1/concepts/glossary/#tag-key) is irrelevant.
...
[tag value](/influxdb/v1.1/concepts/glossary/#tag-value) of `location` in
the `h2o_feet` [measurement](/influxdb/v1.1/concepts/glossary/#measurement).
InfluxDB returns results in two [series](/influxdb/v1.1/concepts/glossary/#series): one for each tag value of `location`.
...
If you request a query that has no timestamp to return, such as an [aggregation function](/influxdb/v1.1/query_language/functions/) with an unbounded time range, InfluxDB returns epoch 0 as the timestamp.
...
The query uses an InfluxQL [function](/influxdb/v1.1/query_language/functions/) to calculate the average `index` for
each combination of the `location` [tag](/influxdb/v1.1/concepts/glossary/#tag) and the `randtag` tag in the
...
[tag](/influxdb/v1.1/concepts/glossary/#tag) combination in the `h2o_quality`
measurement.
...
Basic `GROUP BY time()` queries require an InfluxQL [function](/influxdb/v1.1/query_language/functions/)
...
The query uses an InfluxQL [function](/influxdb/v1.1/query_language/functions/)
to count the number of `water_level` points with the [tag](/influxdb/v1.1/concepts/glossary/#tag)
`location = coyote_creek` and it group results into 12 minute intervals.

The result for each [timestamp](/influxdb/v1.1/concepts/glossary/#timestamp)
...
The query returns two [series](/influxdb/v1.1/concepts/glossary/#series) of results: one for each
[tag value](/influxdb/v1.1/concepts/glossary/#tag-value) of the `location` tag.
...
Advanced `GROUP BY time()` queries require an InfluxQL [function](/influxdb/v1.1/query_language/functions/)
in the [`SELECT` clause](#the-basic-select-statement) and a time range in the
[`WHERE` clause](#the-where-clause).
Note that the `GROUP BY` clause must come after the `WHERE` clause.
...
`GROUP(ing) BY` several things (for example, both [tags](/influxdb/v1.1/concepts/glossary/#tag) and a time interval).
...
The `INTO` clause writes query results to a user-specified [measurement](/influxdb/v1.1/concepts/glossary/#measurement).
...
The `INTO` clause supports several formats for specifying a [measurement](/influxdb/v1.1/concepts/glossary/#measurement):

`INTO <measurement_name>`  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
Writes data to the specified measurement.
If you're using the [CLI](/influxdb/v1.1/tools/shell/) InfluxDB writes the data to the measurement in the
[`USE`d](/influxdb/v1.1/tools/shell/#commands)
[database](/influxdb/v1.1/concepts/glossary/#database) and the `DEFAULT` [retention policy](/influxdb/v1.1/concepts/glossary/#retention-policy-rp).
If you're using the [HTTP API](/influxdb/v1.1/tools/api/) InfluxDB writes the data to the
measurement in the database specified in the [`db` query string parameter](/influxdb/v1.1/tools/api/#query-string-parameters)
and the `DEFAULT` retention policy.
...
The query writes its results a new [measurement](/influxdb/v1.1/concepts/glossary/#measurement): `h2o_feet_copy_1`.
If you're using the [CLI](/influxdb/v1.1/tools/shell/), InfluxDB writes the data to
the `USE`d [database](/influxdb/v1.1/concepts/glossary/#database) and the `DEFAULT` [retention policy](/influxdb/v1.1/concepts/glossary/#retention-policy-rp).
If you're using the [HTTP API](/influxdb/v1.1/tools/api/), InfluxDB writes the
data to the database and retention policy specified in the `db` and `rp`
[query string parameters](/influxdb/v1.1/tools/api/#query-string-parameters).
If you do not set the `rp` query string parameter, the HTTP API automatically
...
query.
See [Database Management](/influxdb/v1.1/query_language/database_management/)
...
InfluxQL [function](/influxdb/v1.1/query_language/functions) and a [`GROUP BY
...
If an `INTO` query includes a [tag key](/influxdb/v1.1/concepts/glossary#tag-key) in the [`SELECT` clause](#the-basic-select-statement), the query converts [tags](/influxdb/v1.1/concepts/glossary#tag) in the current
measurement to [fields](/influxdb/v1.1/concepts/glossary#field) in the destination measurement.
This can cause InfluxDB to overwrite [points](/influxdb/v1.1/concepts/glossary#point) that were previously differentiated
by a [tag value](/influxdb/v1.1/concepts/glossary#tag-value).
The
[Frequently Asked Questions](/influxdb/v1.1/troubleshooting/frequently-asked-questions/#why-are-my-into-queries-missing-data)
...
See the [Continuous Queries](/influxdb/v1.1/query_language/#continuous_queries)
documentation for how to automate `INTO` clause queries on realtime data.
Among [other uses](/influxdb/v1.1/query_language/continuous_queries/#continuous-query-use-cases),
Continuous Queries automate the downsampling process.

<br>
<br>
# ORDER BY time DESC
By default, InfluxDB returns results in ascending time order; the first [point](/influxdb/v1.1/concepts/glossary/#point)
returned has the oldest [timestamp](/influxdb/v1.1/concepts/glossary/#timestamp) and
...
`h2o_feet` [measurement](/influxdb/v1.1/concepts/glossary/#measurement) first.
...
[points](/influxdb/v1.1/concepts/glossary/#point) and the number of
[series](/influxdb/v1.1/concepts/glossary/#series) returned per query.

## The LIMIT Clause
`LIMIT <N>` returns the first `N` [points](/influxdb/v1.1/concepts/glossary/#point) from the specified [measurement](/influxdb/v1.1/concepts/glossary/#measurement).
...
`N` specifies the number of [points](/influxdb/v1.1/concepts/glossary/#point) to return from the specified [measurement](/influxdb/v1.1/concepts/glossary/#measurement).
...
The query returns the three oldest [points](/influxdb/v1.1/concepts/glossary/#point) (determined by timestamp) from the
`h2o_feet` [measurement](/influxdb/v1.1/concepts/glossary/#measurement).
...
to calculate the average `water_level` for each [tag](/influxdb/v1.1/concepts/glossary/#tag) and for each twelve-minute
interval in the query's time range.
`LIMIT 2` requests the two oldest twelve-minute averages (determined by timestamp).

Note that without `LIMIT 2`, the query would return four points per [series](/influxdb/v1.1/concepts/glossary/#series);
one for each twelve-minute interval in the query's time range.

## The `SLIMIT` Clause
`SLIMIT <N>` returns every [point](/influxdb/v1.1/concepts/glossary/#point) from \<N> [series](/influxdb/v1.1/concepts/glossary/#series) in the specified [measurement](/influxdb/v1.1/concepts/glossary/#measurement).
...
### Description of Syntax
`N` specifies the number of [series](/influxdb/v1.1/concepts/glossary/#series) to return from the specified [measurement](/influxdb/v1.1/concepts/glossary/#measurement).
...
The query returns all `water_level` [points](/influxdb/v1.1/concepts/glossary/#points) from one of the [series](/influxdb/v1.1/concepts/glossary/#series) associated
with the `h2o_feet` [measurement](/influxdb/v1.1/concepts/glossary/#measurement).
...
## LIMIT and SLIMIT
`LIMIT <N>` followed by `SLIMIT <N>` returns the first \<N> [points](/influxdb/v1.1/concepts/glossary/#point) from \<N> [series](/influxdb/v1.1/concepts/glossary/#series) in the specified measurement.
...
`N1` specifies the number of [points](/influxdb/v1.1/concepts/glossary/#point) to return per [measurement](/influxdb/v1.1/concepts/glossary/#measurement).
If `N1` is greater than the number of points in a measurement, InfluxDB returns all points from that measurement.

`N2` specifies the number of series to return from the specified [measurement](/influxdb/v1.1/concepts/glossary/#measurement).
...
The query returns the three oldest [points](/influxdb/v1.1/concepts/glossary/#point) (determined by timestamp) from one
of the [series](/influxdb/v1.1/concepts/glossary/#series) associated with the
[measurement](/influxdb/v1.1/concepts/glossary/#measurement) `h2o_feet`.
...
`OFFSET` and `SOFFSET` paginates [points](/influxdb/v1.1/concepts/glossary/#point) and [series](/influxdb/v1.1/concepts/glossary/#series) returned.
...
## The `OFFSET` clause
`OFFSET <N>` paginates `N` [points](/influxdb/v1.1/concepts/glossary/#point) in the query results.
...
### Description of Syntax
`N` specifies the number of [points](/influxdb/v1.1/concepts/glossary/#point) to paginate.
...
The query returns the fourth, fifth, and sixth [points](/influxdb/v1.1/concepts/glossary/#point) from the `h2o_feet` [measurement](/influxdb/v1.1/concepts/glossary/#measurement).
...
The [`SELECT` clause](#the-basic-select-statement) specifies an InfluxQL [function](/influxdb/v1.1/query_language/functions).  
...
## The `SOFFSET` clause
`SOFFSET <N>` paginates `N` [series](/influxdb/v1.1/concepts/glossary/#series) in the query results.
...
### Description of Syntax
`N` specifies the number of [series](/influxdb/v1.1/concepts/glossary/#series) to paginate.
...
The query returns data for the [series](/influxdb/v1.1/concepts/glossary/#series) associated with the `h2o_feet`
[measurement](/influxdb/v1.1/concepts/glossary/#measurement) and the `location = santa_monica` [tag](/influxdb/v1.1/concepts/glossary/#tag).
...
clause. See the [Frequently Asked Questions](/influxdb/v1.1/troubleshooting/frequently-asked-questions/#why-is-my-query-with-a-where-or-time-clause-returning-empty-results)
...
Include a [duration literal](/influxdb/v1.1/query_language/spec/#durations)
...
Add (`+`) or subtract (`-`) a time from a timestamp with a [duration literal](/influxdb/v1.1/query_language/spec/#durations).
...
The query returns data with timestamps that occur between August 18, 2015
at 00:00:00 and August 18, 2015 at 00:12:00.
The `s` [duration literal](/influxdb/v1.1/query_language/spec/#durations) at the
...
## Relative time
Use [`now()`](/influxdb/v1.1/concepts/glossary/#now) to query data with [timestamps](/influxdb/v1.1/concepts/glossary/#timestamp) relative to the server's current timestamp.
...
The whitespace between `-` or `+` and the [duration literal](/influxdb/v1.1/query_language/spec/#durations) is required.
...
The [CLI](/influxdb/v1.1/tools/shell/) returns timestamps in
nanosecond epoch format by default.
Specify alternative formats with the
[`precision <format>` command](/influxdb/v1.1/tools/shell/#influx-commands).
The [HTTP API](/influxdb/v1.1/tools/api/) returns timestamps
in [RFC3339](https://www.ietf.org/rfc/rfc3339.txt) format by default.
Specify alternative formats with the
[`epoch` query string parameter](/influxdb/v1.1/tools/api/#query-string-parameters).
...
The query selects all [field keys](/influxdb/v1.1/concepts/glossary/#field-key)
and [tag keys](/influxdb/v1.1/concepts/glossary/#tag-key) that include an `l`.
...
The query uses an InfluxQL [function](/influxdb/v1.1/query_language/functions/)
to return the distinct [field values](/influxdb/v1.1/concepts/glossary/#field-value)
...
The query uses an InfluxQL [function](/influxdb/v1.1/query_language/functions/)
to calculate the average `degrees` for every [measurement](/influxdb/v1.1/concepts/glossary#measurement) in the `NOAA_water_database`
[database](/influxdb/v1.1/concepts/glossary#database) that contains the word `temperature`.
...
The query uses an InfluxQL [function](/influxdb/v1.1/query_language/functions/)
to calculate the average `water_level` where the [tag value](/influxdb/v1.1/concepts/glossary#tag-value) of `location`
...
[tag](/influxdb/v1.1/concepts/glossary#tag) has no value.
Every data [point](/influxdb/v1.1/concepts/glossary#point) in the `NOAA_water_database` has a tag value for `location`.

It's possible to perform this same query without a regular expression.
See the
[Frequently Asked Questions](/influxdb/v1.1/troubleshooting/frequently-asked-questions/#how-do-i-select-data-with-a-tag-that-has-no-value)
document for more information.
...
The [`SELECT` clause](#the-basic-select-statement) supports specifying a [field's](/influxdb/v1.1/concepts/glossary/#field) type and basic cast
...
[Field values](/influxdb/v1.1/concepts/glossary/#field-value) can be floats, integers, strings, or booleans.
...
In most cases, InfluxDB rejects any writes that attempt to write a [field value](/influxdb/v1.1/concepts/glossary/#field-value)
to a field that previously accepted field values of a different type.
>
It is possible for field value types to differ across [shard groups](/influxdb/v1.1/concepts/glossary/#shard-group).
...
Currently, InfluxDB supports casting [field values](/influxdb/v1.1/concepts/glossary/#field-value) from integers to
...
The query returns the integer form of `water_level`'s float [field values](/influxdb/v1.1/concepts/glossary/#field-value).
...
In InfluxDB, queries merge [series](/influxdb/v1.1/concepts/glossary/#series)
automatically.

### Example

The `h2o_feet` [measurement](/influxdb/v1.1/concepts/glossary/#measurement) in the `NOAA_water_database` is part of two [series](/influxdb/v1.1/concepts/glossary/#series).
The first series is made up of the `h2o_feet` measurement and the `location = coyote_creek` [tag](/influxdb/v1.1/concepts/glossary/#tag).
The second series is made of up the `h2o_feet` measurement and the `location = santa_monica` tag.

The following query automatically merges those two series when it calculates the [average](/influxdb/v1.1/query_language/functions/#mean) `water_level`:
...
In InfluxDB's [CLI](/influxdb/v1.1/tools/shell/):
...
With InfluxDB's [HTTP API](/influxdb/v1.1/tools/api/):
