...
* [The `SELECT` statement and the `WHERE` clause](/influxdb/v1.1/query_language/data_exploration/#the-select-statement-and-the-where-clause)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[The basic `SELECT` statement](/influxdb/v1.1/query_language/data_exploration/#the-basic-select-statement)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[The `SELECT` statement and arithmetic](/influxdb/v1.1/query_language/data_exploration/#the-select-statement-and-arithmetic)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[The `WHERE` clause](/influxdb/v1.1/query_language/data_exploration/#the-where-clause)
* [The `GROUP BY` clause](/influxdb/v1.1/query_language/data_exploration/#the-group-by-clause)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[`GROUP BY` tag values](/influxdb/v1.1/query_language/data_exploration/#group-by-tag-values)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[`GROUP BY` time intervals](/influxdb/v1.1/query_language/data_exploration/#group-by-time-intervals)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[`GROUP BY` tag values and a time interval](/influxdb/v1.1/query_language/data_exploration/#group-by-tag-values-and-a-time-interval)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[The `GROUP BY` clause and `fill()`](/influxdb/v1.1/query_language/data_exploration/#the-group-by-clause-and-fill)  
* [The `INTO` clause](/influxdb/v1.1/query_language/data_exploration/#the-into-clause)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Relocate data](/influxdb/v1.1/query_language/data_exploration/#relocate-data)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Downsample data](/influxdb/v1.1/query_language/data_exploration/#downsample-data)    

Limit and sort your results:

* [Limit query returns with `LIMIT` and `SLIMIT`](/influxdb/v1.1/query_language/data_exploration/#limit-query-returns-with-limit-and-slimit)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Limit results per series with `LIMIT`](/influxdb/v1.1/query_language/data_exploration/#limit-the-number-of-results-returned-per-series-with-limit)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Limit the number of series returned with `SLIMIT`](/influxdb/v1.1/query_language/data_exploration/#limit-the-number-of-series-returned-with-slimit)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Limit the number of points and series returned with `LIMIT` and `SLIMIT`](/influxdb/v1.1/query_language/data_exploration/#limit-the-number-of-points-and-series-returned-with-limit-and-slimit)
* [Sort query returns with `ORDER BY time DESC`](/influxdb/v1.1/query_language/data_exploration/#sort-query-returns-with-order-by-time-desc)
* [Paginate query returns with `OFFSET` and `SOFFSET`](/influxdb/v1.1/query_language/data_exploration/#paginate-query-returns-with-offset-and-soffset)

General tips on query syntax:

* [Multiple statements in queries](/influxdb/v1.1/query_language/data_exploration/#multiple-statements-in-queries)
* [Merge series in queries](/influxdb/v1.1/query_language/data_exploration/#merge-series-in-queries)
* [Time syntax in queries](/influxdb/v1.1/query_language/data_exploration/#time-syntax-in-queries)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Relative time](/influxdb/v1.1/query_language/data_exploration/#relative-time)  
...
* [Regular expressions in queries](/influxdb/v1.1/query_language/data_exploration/#regular-expressions-in-queries)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Regular expressions and selecting measurements](/influxdb/v1.1/query_language/data_exploration/#regular-expressions-and-selecting-measurements)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Regular expressions and specifying tags](/influxdb/v1.1/query_language/data_exploration/#regular-expressions-and-specifying-tags)
...
#### Sample data
<br>
If you'd like to follow along with the queries in this document, see [Sample Data](/influxdb/v1.1/query_language/data_download/) for how to download and write the data to InfluxDB.
...
The [series](/influxdb/v1.1/concepts/glossary/#series) are made up of the [measurement](/influxdb/v1.1/concepts/glossary/#measurement) `h2o_feet` and the [tag key](/influxdb/v1.1/concepts/glossary/#tag-key) `location` with the [tag values](/influxdb/v1.1/concepts/glossary/#tag-value) `santa_monica` and `coyote_creek`.
There are two [fields](/influxdb/v1.1/concepts/glossary/#field): `water_level` which stores floats and `level description` which stores strings.
All of the data are in the `NOAA_water_database` database.

> **Disclaimer:** The `level description` field isn't part of the original NOAA data - we snuck it in there for the sake of having a field key with a special character and string [field values](/influxdb/v1.1/concepts/glossary/#field-value).
...
the same name.
In addition, you can specify a field's [type](/influxdb/v1.1/write_protocols/write_syntax/#data-types) with the syntax
`<field_key>::<type>`.
...
[retention policy](/influxdb/v1.1/concepts/glossary/#retention-policy-rp) in
...
This can lead to [overflow issues](/influxdb/v1.1/troubleshooting/frequently-asked-questions/#what-are-the-minimum-and-maximum-integers-that-influxdb-can-store) for some numbers.
...
Return data where the tag key `location` has no tag value (more on regular expressions [later](/influxdb/v1.1/query_language/data_exploration/#regular-expressions-in-queries)):
```sql
...
For more on `now()` and other ways to specify time in queries, see [time syntax in queries](/influxdb/v1.1/query_language/data_exploration/#time-syntax-in-queries).
...
To successfully implement `GROUP BY`,  append the`GROUP BY` clause to a `SELECT` statement and pair the `SELECT` statement with one of InfluxQL's [functions](/influxdb/v1.1/query_language/functions/):
...
### GROUP BY tag values
Calculate the [`MEAN()`](/influxdb/v1.1/query_language/functions/#mean) `water_level` for the different tag values of `location`:
```sql
...
Calculate the [`MEAN()`](/influxdb/v1.1/query_language/functions/#mean) `index` for every tag set in `h2o_quality`:
```sql
...
[`COUNT()`](/influxdb/v1.1/query_language/functions/#count) the number of `water_level` points between August 19, 2015 at midnight and August 27 at 5:00pm at three day intervals:
```sql
...
See [Frequently Asked Questions](/influxdb/v1.1/troubleshooting/frequently-asked-questions/#what-determines-the-time-intervals-returned-by-group-by-time-queries) for a more detailed explanation of the default
...
[`COUNT()`](/influxdb/v1.1/query_language/functions/#count) the number of `water_level` points between August 19, 2015 at midnight and August 27 at 5:00pm at three day intervals, and offset
...
[`COUNT()`](/influxdb/v1.1/query_language/functions/#count) the number of
...
* `fill(previous)` doesn’t fill the result for a time bucket if the previous value is outside the query’s time range. See [Frequently Asked Questions](/influxdb/v1.1/troubleshooting/frequently-asked-questions/#why-does-fill-previous-return-empty-results) for more information.
...
Write the field `water_level` in `h2o_feet` to a new measurement (`h2o_feet_copy`) and to the retention policy `autogen` in the [already-existing](/influxdb/v1.1/query_language/database_management/#create-a-database-with-create-database) database `where_else`:
```sql
...
### Downsample data
Combine the `INTO` clause with an InfluxQL [function](/influxdb/v1.1/query_language/functions/) and a `GROUP BY` clause to write the lower precision query results to a different measurement:
```sql
...
If you want InfluxDB to automatically query and downsample all future data see [Continuous Queries](/influxdb/v1.1/query_language/continuous_queries/).
...
Use [fill()](/influxdb/v1.1/query_language/data_exploration/#the-group-by-clause-and-fill) to ensure that every time interval appears in the results.
...
* `LIMIT <N>` returns the first \<N> [points](/influxdb/v1.1/concepts/glossary/#point) from each [series](/influxdb/v1.1/concepts/glossary/#series) in the specified measurement.
...
The `NOAA_water_database` database has two [series](/influxdb/v1.1/concepts/glossary/#series).
...
in the `WHERE` clause.
See [Frequently Asked Questions](/influxdb/v1.1/troubleshooting/frequently-asked-questions/#why-is-my-query-with-a-where-or-time-clause-returning-empty-results)
...
In this section we'll be using all of the measurements in the [sample data](/influxdb/v1.1/query_language/data_exploration/#sample-data):
...
See [the WHERE clause](/influxdb/v1.1/query_language/data_exploration/#the-where-clause) section for an example of how to return data where a tag key has a value and an example of how to return data where a tag key has no value using regular expressions.
