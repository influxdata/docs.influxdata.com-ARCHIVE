...
There are many ways to write data into InfluxDB including the [command line interface](/influxdb/v0.11/tools/shell/), [client libraries](/influxdb/v0.11/clients/api/) and plugins for common data formats such as [Graphite](/influxdb/v0.11/write_protocols/graphite/).
...
The data consist of the [measurement](/influxdb/v0.11/concepts/glossary/#measurement) `cpu_load_short`, the [tag keys](/influxdb/v0.11/concepts/glossary/#tag-key) `host` and `region` with the [tag values](/influxdb/v0.11/concepts/glossary/#tag-value) `server01` and `us-west`, the [field key](/influxdb/v0.11/concepts/glossary/#field-key) `value` with a [field value](/influxdb/v0.11/concepts/glossary/#field-value) of `0.64`, and the [timestamp](/influxdb/v0.11/concepts/glossary/#timestamp) `1434055562000000000`.
<br>
...
Field keys are required and are always strings, and, [by default](/influxdb/v0.11/write_protocols/write_syntax/#line-protocol), field values are floats.
...
The data in the file should follow InfluxDB's [line protocol syntax](/influxdb/v0.11/write_protocols/write_syntax/).
...
[By default](/influxdb/v0.11/administration/config/#cluster), the HTTP request times out after five seconds.
...
### Next steps
---
Now that you know how to write data with the built-in HTTP API discover how to query them with the [Querying Data](/influxdb/v0.11/guides/querying_data/) guide!
