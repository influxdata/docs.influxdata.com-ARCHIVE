...
There are many ways to write data into InfluxDB including the [command line interface](/influxdb/v0.13/tools/shell/), [client libraries](/influxdb/v0.13/clients/api/) and plugins for common data formats such as [Graphite](/influxdb/v0.13/write_protocols/graphite/).
...
The data consist of the [measurement](/influxdb/v0.13/concepts/glossary/#measurement) `cpu_load_short`, the [tag keys](/influxdb/v0.13/concepts/glossary/#tag-key) `host` and `region` with the [tag values](/influxdb/v0.13/concepts/glossary/#tag-value) `server01` and `us-west`, the [field key](/influxdb/v0.13/concepts/glossary/#field-key) `value` with a [field value](/influxdb/v0.13/concepts/glossary/#field-value) of `0.64`, and the [timestamp](/influxdb/v0.13/concepts/glossary/#timestamp) `1434055562000000000`.
<br>
...
See the [HTTP section](/influxdb/v0.13/write_protocols/write_syntax/#http) on the Write Syntax page for a complete list of the available query parameters.

The body of the POST - we call this the [Line Protocol](/influxdb/v0.13/concepts/glossary/#line-protocol) - contains the time-series data that you wish to store.
...
The data in the file should follow InfluxDB's [line protocol syntax](/influxdb/v0.13/write_protocols/write_syntax/).
...
This simple, easy to understand system uses HTTP as a transfer system for [InfluxQL](/influxdb/v0.13/query_language/spec/).
...
### Next steps
---
Now that you know how to write data with the built-in HTTP API discover how to query them with the [Querying Data](/influxdb/v0.13/guides/querying_data/) guide!
