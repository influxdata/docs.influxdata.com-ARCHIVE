...
InfluxDB will create a [shard](/influxdb/v1.2/concepts/glossary/#shard) for each block of time.
For example, if you have a [retention policy](/influxdb/v1.2/concepts/glossary/#retention-policy) with an unlimited duration, shards will be created for each 7 day block of time.
Each of these shards maps to an underlying storage engine database.
Each of these databases has its own [WAL](/influxdb/v1.2/concepts/glossary/#wal-write-ahead-log) and TSM files.
...
* In-Memory Index - The in-memory index is a shared index across shards that provides the quick access to [measurements](/influxdb/v1.2/concepts/glossary/#measurement), [tags](/influxdb/v1.2/concepts/glossary/#tags), and [series](/influxdb/v1.2/concepts/glossary/#series).  The index is used by the engine, but is not specific to the storage engine itself.
...
The points are organized by the key, which is the measurement, [tag set](/influxdb/v1.2/concepts/glossary/#tag-set), and unique [field](/influxdb/v1.2/concepts/glossary/#field).
...
There is a lower bound, [`cache-snapshot-memory-size`](/influxdb/v1.2/administration/config/#cache-snapshot-memory-size-26214400), which when exceeded will trigger a snapshot to TSM files and remove the corresponding WAL segments.
There is also an upper bound, [`cache-max-memory-size`](/influxdb/v1.2/administration/config/#cache-max-memory-size-524288000), which when exceeded will cause the Cache to reject new writes.
...
The idle threshold, [`cache-snapshot-write-cold-duration`](/influxdb/v1.2/administration/config/#cache-snapshot-write-cold-duration-1h0m0s), forces the Cache to snapshot to TSM files if it hasn't received a write within the specified interval.
