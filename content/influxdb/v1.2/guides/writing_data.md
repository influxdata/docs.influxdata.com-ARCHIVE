...
There are many ways to write data into InfluxDB including the [command line interface](/influxdb/v1.2/tools/shell/), [client libraries](/influxdb/v1.2/clients/api/) and plugins for common data formats such as [Graphite](/influxdb/v1.2/write_protocols/graphite/).
...
The data consist of the [measurement](/influxdb/v1.2/concepts/glossary/#measurement) `cpu_load_short`, the [tag keys](/influxdb/v1.2/concepts/glossary/#tag-key) `host` and `region` with the [tag values](/influxdb/v1.2/concepts/glossary/#tag-value) `server01` and `us-west`, the [field key](/influxdb/v1.2/concepts/glossary/#field-key) `value` with a [field value](/influxdb/v1.2/concepts/glossary/#field-value) of `0.64`, and the [timestamp](/influxdb/v1.2/concepts/glossary/#timestamp) `1434055562000000000`.
<br>
...
See the [API Reference](/influxdb/v1.2/tools/api/#write) documentation for a complete list of the available query parameters.

The body of the POST - we call this the [Line Protocol](/influxdb/v1.2/concepts/glossary/#line-protocol) - contains the time-series data that you wish to store.
...
Field keys are required and are always strings, and, [by default](/influxdb/v1.2/write_protocols/line_protocol_reference/#data-types), field values are floats.
...
The data in the file should follow InfluxDB's [line protocol syntax](/influxdb/v1.2/write_protocols/write_syntax/).
...
This simple, easy to understand system uses HTTP as a transfer method for [InfluxQL](/influxdb/v1.2/query_language/spec/).
