...
[function](/influxdb/v1.2/concepts/glossary/#function),
an [`INTO` clause](/influxdb/v1.2/query_language/spec/#clauses),
and a [`GROUP BY time()` clause](/influxdb/v1.2/query_language/spec/#clauses):
...
[`now()`](/influxdb/v1.2/concepts/glossary/#now) and `now()` minus the
...
<br>
[Fully qualify](/influxdb/v1.2/query_language/data_exploration/#the-basic-select-statement)
the destination measurement to store the downsampled data in a non-`DEFAULT`
[retention policy](/influxdb/v1.2/concepts/glossary/#retention-policy-rp) (RP).
...
See the [Downsampling and Data Retention](/influxdb/v1.2/guides/downsampling_and_retention/)
...
[backreferencing syntax](/influxdb/v1.2/query_language/data_exploration/#the-into-clause)
...
[`fill()`](/influxdb/v1.2/query_language/data_exploration/#group-by-time-intervals-and-fill)
...
relative to [`now()`](/influxdb/v1.2/concepts/glossary/#now).
Use a basic
[`INTO` query](/influxdb/v1.2/query_language/data_exploration/#the-into-clause)
to backfill results for data with older timestamps.

##### Issue 4: Missing tags in the CQ results
<br>
By default, all
[`INTO` queries](/influxdb/v1.2/query_language/data_exploration/#the-into-clause)
...
##### The cq_query
<br>
See [ Description of Basic Syntax](/influxdb/v1.2/query_language/continuous_queries/#description-of-basic-syntax).
...
When the CQ executes, it runs a single query for the time range between
[`now()`](/influxdb/v1.2/concepts/glossary/#now) and `now()` minus the `FOR` interval in the `RESAMPLE` clause.
...
[duration literals](/influxdb/v1.2/query_language/spec/#durations).
The `RESAMPLE` clause works with either or both of the `EVERY` and `FOR` intervals
configured.
CQs default to the relevant
[basic syntax behavior](/influxdb/v1.2/query_language/continuous_queries/#description-of-basic-syntax)
if the `EVERY` interval or `FOR` interval is not provided (see the first issue in
[Common Issues with Advanced Syntax](/influxdb/v1.2/query_language/continuous_queries/#common-issues-with-advanced-syntax)
...
[handles duplicate points](/influxdb/v1.2/troubleshooting/frequently-asked-questions/#how-does-influxdb-handle-duplicate-points)
...
See [Frequently Asked Questions](/influxdb/v1.2/troubleshooting/frequently-asked-questions/#why-does-fill-previous-return-empty-results)
...
Only admin users are allowed to work with CQs. For more on user privileges, see [Authentication and Authorization](/influxdb/v1.2/query_language/authentication_and_authorization/#user-types-and-their-privileges).
...
[retention policies](/influxdb/v1.2/concepts/glossary/#retention-policy-rp)
...
[support nesting](/influxdb/v1.2/troubleshooting/frequently-asked-questions/#which-influxql-functions-support-nesting)
of other functions.
Most do not.
...
InfluxDB's CQs.
Check out the
[Kapacitor documentation](/kapacitor/v1.2/examples/continuous_queries/) for when
