...
Single quote [tag values](/influxdb/v1.2/concepts/glossary/#tag-value) in
the `WHERE` clause.
...
The query returns the three oldest [points](/influxdb/v1.2/concepts/glossary/#point) (determined by timestamp) from one
...
Include a [duration literal](/influxdb/v1.2/query_language/spec/#durations)
...
Add (`+`) or subtract (`-`) a time from a timestamp with a [duration literal](/influxdb/v1.2/query_language/spec/#durations).
...
The query returns data with timestamps that occur between August 18, 2015
at 00:00:00 and August 18, 2015 at 00:12:00.
The `s` [duration literal](/influxdb/v1.2/query_language/spec/#durations) at the
...
## Relative time
Use [`now()`](/influxdb/v1.2/concepts/glossary/#now) to query data with [timestamps](/influxdb/v1.2/concepts/glossary/#timestamp) relative to the server's current timestamp.
...
The whitespace between `-` or `+` and the [duration literal](/influxdb/v1.2/query_language/spec/#durations) is required.
...
See the [Frequently Asked Questions](/influxdb/v1.2/troubleshooting/frequently-asked-questions/#why-is-my-query-with-a-where-or-time-clause-returning-empty-results)
...
Use the [CLI](/influxdb/v1.2/tools/shell/) to write a point to the `NOAA_water_database` that occurs after `now()`:
```
...
The [CLI](/influxdb/v1.2/tools/shell/) returns timestamps in
nanosecond epoch format by default.
Specify alternative formats with the
[`precision <format>` command](/influxdb/v1.2/tools/shell/#influx-commands).
The [HTTP API](/influxdb/v1.2/tools/api/) returns timestamps
in [RFC3339](https://www.ietf.org/rfc/rfc3339.txt) format by default.
Specify alternative formats with the
[`epoch` query string parameter](/influxdb/v1.2/tools/api/#query-string-parameters).
...
The query selects all [field keys](/influxdb/v1.2/concepts/glossary/#field-key)
and [tag keys](/influxdb/v1.2/concepts/glossary/#tag-key) that include an `l`.
...
[tag](/influxdb/v1.2/concepts/glossary#tag) has no value.
Every data [point](/influxdb/v1.2/concepts/glossary#point) in the `NOAA_water_database` has a tag value for `location`.
...
[Field values](/influxdb/v1.2/concepts/glossary/#field-value) can be floats, integers, strings, or booleans.
...
In most cases, InfluxDB rejects any writes that attempt to write a [field value](/influxdb/v1.2/concepts/glossary/#field-value)
to a field that previously accepted field values of a different type.
>
It is possible for field value types to differ across [shard groups](/influxdb/v1.2/concepts/glossary/#shard-group).
...
Currently, InfluxDB supports casting [field values](/influxdb/v1.2/concepts/glossary/#field-value) from integers to
...
The query returns the integer form of `water_level`'s float [field values](/influxdb/v1.2/concepts/glossary/#field-value).
...
The `h2o_feet` [measurement](/influxdb/v1.2/concepts/glossary/#measurement) in the `NOAA_water_database` is part of two [series](/influxdb/v1.2/concepts/glossary/#series).
The first series is made up of the `h2o_feet` measurement and the `location = coyote_creek` [tag](/influxdb/v1.2/concepts/glossary/#tag).
The second series is made of up the `h2o_feet` measurement and the `location = santa_monica` tag.

The following query automatically merges those two series when it calculates the [average](/influxdb/v1.2/query_language/functions/#mean) `water_level`:
...
In InfluxDB's [CLI](/influxdb/v1.2/tools/shell/):
